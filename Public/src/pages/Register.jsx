...
const Register = () => {
    ...
    const handleFormValidation = () => {
        ...
        if (username.length < 3) {
            toast.error("Length of the username should be greater than 3", {
                position: "bottom-right",
                autoClose: 8000,
                pauseOnHover: true,
                theme:"dark",
  ...
  return (
      <div className='flex flex-col w-full h-full color bg-slate-500 justify-center items-center gap-0 '>
          <form onSubmit={handleSubmit} className='gap-2 flex flex-col bg-slate-950 rounded-md p-10'>
              ...
              <input className='bg-transparent p-3 border-2 text-white' type="email" placeholder='Email' onChange={handleChange} value={user.email} name='email' />
              ...
              <input className='bg-transparent p-3 border-2 text-white' type="password" placeholder='ConfirmPassword' onChange={handleChange} value={user.confirmPassword} name='confirmPassword' /> 
              <button className='bg-green-600 p-3' type='submit'>Sign up</button>
              <span className='text-white uppercase'>AllReady have a account? <Link to="/login" className='text-blue-800 '>Login</Link></span>
